...
function normalizeHttpsMatch(raw) {
...
/**
 * Pull unique https:// URLs from free text or CSV file contents.
 * @param {string} text
 * @returns {string[]}
 */
export function extractHttpsUrls(text) {
  ...
  const seen = new Set();
  const out = [];
  const re = new RegExp(HTTPS_URL_PATTERN.source, "gi");
  let match;
  while ((match = re.exec(text)) !== null) {
    const href = normalizeHttpsMatch(match[0]);
...
export const MAX_BATCH_SOURCE_URLS = 50;
